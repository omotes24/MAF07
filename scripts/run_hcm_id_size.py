...
from maf07.methods.hcm import hcm_from_env
...
DEFAULT_VARIANTS = "maha_calib,hcm_calib,hcm_raw"
...
def _variant_config(name: str) -> tuple[float, bool]:
    if name == "maha_calib":
        return 1e9, True
    if name == "hcm_calib":
        return float(os.environ.get("MAF07_HCM_DELTA", "1.345")), True
    if name == "hcm_raw":
        return float(os.environ.get("MAF07_HCM_DELTA", "1.345")), False
    raise ValueError(f"Unknown HCM variant: {name}")
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--variants", default=os.environ.get("MAF07_HCM_VARIANTS", DEFAULT_VARIANTS))
    ...
    output = resolve_path(args.output or f"results/quick/hcm_id_size{args.id_size}_{protocol}_results.csv")
    summary_output = resolve_path(args.summary_output or f"results/quick/hcm_id_size{args.id_size}_{protocol}_summary.csv")
    ...
    for _, base in base_jobs.iterrows():
        ...
        detector = hcm_from_env().fit(train_x, train_y, z_cal=val_x, y_cal=val_y)
        ...
        for _, job in variant_jobs.iterrows():
            ...
            scores = detector.id_scores(eval_x, eval_logits, delta=delta, calibrated=calibrated)
