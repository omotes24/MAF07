...
export MAF07_DIAGCARD_VARIANTS="${MAF07_DIAGCARD_VARIANTS:-diag_calib,diag_huber_calib,diag_raw,diag_huber_raw}"
export MAF07_DIAGCARD_K="${MAF07_DIAGCARD_K:-150}"
export MAF07_DIAGCARD_DELTA="${MAF07_DIAGCARD_DELTA:-1.345}"
export MAF07_DIAGCARD_NORMALIZE="${MAF07_DIAGCARD_NORMALIZE:-0}"
export MAF07_DIAGCARD_MIN_STD="${MAF07_DIAGCARD_MIN_STD:-1e-3}"
export MAF07_DIAGCARD_SCORE_BATCH="${MAF07_DIAGCARD_SCORE_BATCH:-96}"
...
echo "DiagCARD 2x2 full start $(date -u +%Y-%m-%dT%H:%M:%SZ)"
echo "backbones=$MAF07_BACKBONES seeds=$MAF07_SEEDS workers=$WORKERS variants=$MAF07_DIAGCARD_VARIANTS"
...
run_group() {
  ...
  local pids=()
  local devices=(${MAF07_DIAGCARD_CUDA_DEVICES:-0 1 2 3})
  for ((wi=0; wi<WORKERS; wi++)); do
    (
      if [ "${MAF07_DIAGCARD_SET_CUDA_VISIBLE:-1}" = "1" ]; then
        export CUDA_VISIBLE_DEVICES="${devices[$((wi % ${#devices[@]}))]}"
        export MAF07_DIAGCARD_DEVICE="${MAF07_DIAGCARD_DEVICE:-cuda}"
      fi
      nice -n 10 "$PYBIN" scripts/run_diagcard_id_size.py \
        --id-size "$idsize" \
        --protocol "$protocol" \
        --worker-index "$wi" \
