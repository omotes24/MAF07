...
from maf07.methods.csr import csr_from_env
...
def main(argv: list[str] | None = None) -> int:
    ...
    output = resolve_path(args.output or f"results/quick/csr_id_size{args.id_size}_{protocol}_results.csv")
    summary_output = resolve_path(args.summary_output or f"results/quick/csr_id_size{args.id_size}_{protocol}_summary.csv")
    ...
    for _, job in jobs.iterrows():
        ...
        scores = csr_from_env().fit(train_x, train_y, z_cal=val_x, y_cal=val_y).id_scores(eval_x, eval_logits)
